...
port number). The IP address is 127.0.0.1 and the port number is 8000.

2. Is it UDP or TCP? How do you know?
  TCP because in socket(), SOCK_STREAM is passed in as an argument which is used
for TCP.

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code?
  It gets this data from STDIN_FILENO because in the read() call, it reads from
STDIN_FILENO.

4. How does the client program end? How can you tell that in the code?
  The client program ends when 1 or less than 1 byte is read because the while()
loop checks for more than 1 byte read from read().
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buffer, BUF_SIZE)) > 0) {
    pthread_mutex_lock(&count_mutex);
    total_message_count++;
    pthread_mutex_unlock(&count_mutex);
    buffer[num_read] = '\0';
    printf("Msg #\t%d; Client ID %d: %s", total_message_count, client_id,
           buffer);
  }
...
int main() {
  ...
  for (;;) {
    ...
    struct client_info *client_args = malloc(sizeof(struct client_info));

    client_args->cfd = cfd;
    pthread_mutex_lock(&client_id_mutex);
    client_args->client_id = client_id_counter++;
    ...
    // client_info
    pthread_create(&client_thread, NULL, handle_client, client_args);
