...
1. How is the client sending data to the server? What protocol?
  The client loops through a messages array and writes it to the server file
descriptor using TCP.

2. What data is the client sending to the server?
  The client is sending strings to the server.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  A struct containing a flag to run the client or not, a list of messages, and a
mutex for the messages list.

2. How are received messages stored?
  The received messaged are stored in a linked list.

3. What does `main()` do with the received messages?
  main() prints all the received messages.

4. How are threads used in this sample code?
  Threads are used to create a thread which creates a new thread for each
client. This client thread accepts messages.
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
int main() {
  ...
  while (1) {
    ...
    uint32_t num_messages = list_handle.count;
    pthread_mutex_unlock(&list_mutex);
    if (num_messages >= MAX_CLIENTS * NUM_MSG_PER_CLIENT) {
      break;
    }
  }
