...
void handle_error(char *err) { perror(err); }
...
int main() {
  ...
  // Initialize data
  // Start filling in data after the header
  memset(first_block + 1, 0, 128 - sizeof(struct header));
  memset(second_block + 1, 1, 128 - sizeof(struct header));

  // Print addresses of memory blocks
  print_out("first block: %p\n", &first_block, sizeof(&first_block));
  print_out("second block: %p\n", &second_block, sizeof(&second_block));

  // Print size and next address of first block
  print_out("first block size: %d\n", &first_block->size,
            sizeof(&first_block->size));
  print_out("first block next: %p\n", &first_block->next,
            sizeof(&first_block->next));

  // Print size and next address of second block
  print_out("second block size: %d\n", &second_block->size,
            sizeof(&second_block->size));
  print_out("second block next: %p\n", &second_block->next,
            sizeof(&second_block->next));

  // Print values of blocks
  // Start printing after the header and increment by 1 byte
  for (uint64_t i = sizeof(struct header); i < 128; i++) {
    uint64_t value = ((char *)first_block)[i];
    print_out("%d\n", &value, sizeof(&value));
  }
  for (uint64_t i = sizeof(struct header); i < 128; i++) {
    uint64_t value = ((char *)second_block)[i];
    print_out("%d\n", &value, sizeof(&value));
  }
  return 0;
}
